...
public class MoveVGroupCommand extends MoveElementCommand {
	...
	public MoveVGroupCommand(ERDiagram diagram, int x, int y, int width,
			int height, VGroup category, List<VGroup> otherCategories,
			boolean move) {
		...
		if (!this.move) {
			...
			for (Iterator<NodeElement> iter = this.nodeElementList.iterator(); iter
				...
				for (VGroup otherCategory : otherCategories) {
